...
struct fat_bpb { /* BIOS Parameter Block */
    ...
    /* Campos adicionais para FAT32 */
    uint32_t sect_per_fat32;    /* Setores por FAT em FAT32 */
    uint32_t root_cluster;      /* Cluster inicial do diretório raiz */
...
//novos métodos 
uint32_t bpb_fdata_sector_count(struct fat_bpb *bpb);//No FAT32, o campo sect_per_fat32 substitui sect_per_fat:


//métodos para conversar com 
// void read_data(FILE *fp, uint32_t cluster, void *buffer, size_t size);
// uint32_t get_next_cluster_from_fat(FILE *fp, uint32_t cluster, struct fat_bpb *bpb);
// uint32_t get_next_cluster(FILE *fp, uint32_t cluster, struct fat_bpb *bpb);
// 
///
...
//No FAT32, os valores de EOF são mais amplos, pois o FAT32 usa números de 32 bits para os clusters:
